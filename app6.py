...
def fetch_data(sql_query, conn_config):
    conn = snowflake.connector.connect(
        ...
        url=conn_config.get('url', ''),  # Use URL if provided, else use default account format
...
def run_query(query_config, connections):
    ...
    return fetch_data(sql, conn_config)
    ...
    with tab2:
        ...
        for i, query in enumerate(st.session_state['queries']):
            with st.expander(f"{query['group']} - {query['name']}"):
                ...
                query['base_sql'] = st.text_area(f"SQL Query for Query {i+1}", value=query['base_sql'], key=f"sql_{i}")
                query['filter'] = st.text_input(f"Filter Logic for Query {i+1} (OPTIONAL)", value=query['filter'], key=f"filter_{i}")
                ...
                query['group'] = st.selectbox(
                    ...
                    index=st.session_state['groups'].index(query['group']) if query['group'] in st.session_state['groups'] else 0,
        ...
        if st.button("Export Queries"):
            export_data = [{'group': q['group'], 'name': q['name'], 'base_sql': q['base_sql'], 'filter': q['filter'], 'connection_id': q['connection_id'], 'tag': q['tag']} for q in st.session_state['queries']]
    ...
    with tab3:
        ...
        for i, conn in enumerate(st.session_state['connections']):
            with st.expander(f"Connection {i+1}: {conn['name']}"):
                ...
                conn['password'] = st.text_input(f"Password {i+1}", type="password", value=conn['password'], key=f"password_{i}")
        ...
        if st.button("Export Snowflake Configurations"):
            export_connections = [{key: conn[key] for key in conn if key not in ('user', 'password')} for conn in st.session_state['connections']]
